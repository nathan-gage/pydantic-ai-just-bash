...
- Deferred tools stay out of shell introspection until discovered with `bash_search_tools`, and discovered commands remain available across later run steps until you call `bash(..., reset_session=True)`.
...
- before discovery, hidden commands do not appear in shell introspection such as `bash_list_tools` or `type -t`
